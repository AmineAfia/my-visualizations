...
# ----------------------Data formating-----------------------
with open('stars_plot/stars.json') as data_file:
...
# format the ticks
plt.subplot(121).xaxis.set_major_locator(years)
plt.subplot(121).xaxis.set_major_formatter(yearsFmt)
plt.subplot(121).xaxis.set_minor_locator(months)

datemin = datetime.date(angular_df["date"].min().year, 1, 1)
datemax = datetime.date(angular_df["date"].max().year + 1, 1, 1)
plt.subplot(121).set_xlim(datemin, datemax)

# format the coords message box
def price(x):
    return '$%1.2f' % x
plt.subplot(121).format_xdata = mdates.DateFormatter('%Y-%m-%d')
plt.subplot(121).format_ydata = price
plt.subplot(121).grid(True)
